...
void compare_buffer(unsigned char *src, unsigned char *des, int nBytes)
{
    ...
    for (i = 0; i < nBytes; i++)
    {
        if (src[i] != des[i])
        {
            printf_tiny("error idx : %4d : 0x%2X , 0x%2X\r\n", i , src[i],des[i]);
	...
	{
    	printf_tiny("compare_buffer complete \r\n");	
	}

    printf_tiny("compare_buffer end \r\n");
...
void dump_buffer(unsigned char *pucBuff, int nBytes)
{
    ...
    printf_tiny("dump_buffer : %2d\r\n" , nBytes);    
    for (i = 0 ; i < nBytes ; i++)
    {
        printf_tiny("0x%02X," , pucBuff[i]);
        if ((i+1)%8 ==0)
        {
            printf_tiny("\r\n");
        }            
...
void dump_buffer_hex(unsigned char *pucBuff, int nBytes)
{
    ...
    while (nBytes > 0)
    {
        printf_tiny("0x%04X  ", nIdx);
        for (i = 0; i < 16; i++)
            printf_tiny("%02X ", pucBuff[nIdx + i]);
        printf_tiny("  ");
        for (i = 0; i < 16; i++)
        {
            if ((pucBuff[nIdx + i] >= 0x20) && (pucBuff[nIdx + i] < 127))
                printf_tiny("%c", pucBuff[nIdx + i]);
            else
                printf_tiny(".");
            nBytes--;
        }
